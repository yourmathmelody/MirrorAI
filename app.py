...
# Motoru Başlatan Akıllı Fonksiyon
@st.cache_resource
def load_ai_engine():
    try:
        genai.configure(api_key=API_KEY)
        # 404 hatalarını engellemek için tüm isimleri deniyoruz
        for model_name in ['gemini-1.5-flash', 'models/gemini-1.5-flash', 'gemini-pro']:
            try:
                m = genai.GenerativeModel(model_name)
                # Küçük bir bağlantı testi
...
with col1:
    ...
    if st.button("ANALİZİ VE KOÇLUĞU BAŞLAT"):
        if model_engine is None:
            # Motor çalışmazsa nedenini söyleyen kısım burası
            st.error("⚠️ Yapay Zeka Motoru Bağlanamadı! Lütfen API anahtarını veya interneti kontrol edin.")
        elif
